...
pub fn load_string<'gc>(ctx: Context<'gc>) {
    ...
    if matches!(std::env::var("STACK").as_deref(), Ok("1" | "true")) {
        load_pattern_stack(ctx, string)
    } else {
        load_pattern_seq(ctx, string)
    }

    ctx.set_global("string", string);
}

pub fn load_pattern_stack<'gc>(ctx: Context<'gc>, string: Table<'gc>) {
    string.set_field(
        ctx,
        "find",
        Callback::from_fn(&ctx, |ctx, _, mut stack| {
...
pub fn load_pattern_seq<'gc>(ctx: Context<'gc>, string: Table<'gc>) {
    string.set_field(
        ctx,
        "find",
